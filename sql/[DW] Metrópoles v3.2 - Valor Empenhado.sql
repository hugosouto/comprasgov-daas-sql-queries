select	
	...
	a100.ID_ORGAO,
	a100.NO_ORGAO,
	a20.CH_EMPN_EMPENHO EMPENHO,
	sum(a100.EMPENHADO2020)  			EMPENHADO2020,
...
--	  on 	(a14.ID_EMPN_ESPECIE_EMPENHO = a17.ID_EMPN_ESPECIE_EMPENHO)
	...
	join	Siasg_DW_VBL.D_EMPN_EMPENHO          		a20
	  on 	a11.ID_EMPN_EMPENHO = a20.ID_EMPN_EMPENHO
	join	(
			SELECT	
					a111.DS_ITCP_PADRAO_DESC_MAT				ITEM_PDM,
...
--						'PEIXE CONSERVA', 'PEIXE EM CONSERVA')	ITEM_PDM,
					...
					a112.DS_UNDD_PODER  						PODER,
					a113.DS_UNDD_TP_ADM  						TIPO_ADM,
					a110.DS_UNDD_ORGAO_SUP  					NO_ORGAO_SUP,
					a115.ID_UNDD_ORGAO  						ID_ORGAO,
					a210.DS_UNDD_ORGAO  						NO_ORGAO,
					a300.CH_EMPN_EMPENHO						EMPENHO,
				...
				FROM	
					...
					join	Siasg_DW_VBL.D_UNDD_UNIDADE					a115
					  on 	(a11.ID_UNDD_EMITENTE_EMPENHO = a115.ID_UNDD_UNIDADE)
					join	Siasg_DW_VBL.D_UNDD_ORGAO					a210
					  on 	(a115.ID_UNDD_ORGAO = a210.ID_UNDD_ORGAO)
					join	Siasg_DW_VBL.D_EMPN_EMPENHO          		a300
					  on 	a11.ID_EMPN_EMPENHO = a300.ID_EMPN_EMPENHO
				WHERE	
					(a12.ID_DT_ANO IN (2020)	-- (2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020)
--				 	AND (
--						a111.DS_ITCP_PADRAO_DESC_MAT LIKE 'ABÓBORA IN%NATURA'
...
--					REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(a111.DS_ITCP_PADRAO_DESC_MAT, '"', ''), '*', ''), 'DE PIPOCA', 'PIPOCA'), ' - ', ' '), 'PORCO', 'SUÍNA'), 'FARINHA TRIGO', 'FARINHA DE TRIGO'), 'FARINHA MILHO', 'FARINHA DE MILHO'), 'FARINHA AVEIA', 'FARINHA DE AVEIA'), 'FARINHA MANDIOCA', 'FARINHA DE MANDIOCA'), 'LEITE PÓ', 'LEITE EM PÓ'), 'ÁGUA MINERAL NATURAL','ÁGUA MINERAL'), 'ÁGUA MINERAL', 'ÁGUA MINERAL NATURAL'), 'IOGURTE NATURAL', 'IOGURTE'), 'IOGURTE', 'IOGURTE NATURAL'), 'PEIXE CONSERVA', 'PEIXE EM CONSERVA'),
				...
				ORDER BY
					a111.DS_ITCP_PADRAO_DESC_MAT
--					REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(a111.DS_ITCP_PADRAO_DESC_MAT, '"', ''), '*', ''), 'DE PIPOCA', 'PIPOCA'), ' - ', ' '), 'PORCO', 'SUÍNA'), 'FARINHA TRIGO', 'FARINHA DE TRIGO'), 'FARINHA MILHO', 'FARINHA DE MILHO'), 'FARINHA AVEIA', 'FARINHA DE AVEIA'), 'FARINHA MANDIOCA', 'FARINHA DE MANDIOCA'), 'LEITE PÓ', 'LEITE EM PÓ'), 'ÁGUA MINERAL NATURAL','ÁGUA MINERAL'), 'ÁGUA MINERAL', 'ÁGUA MINERAL NATURAL'), 'IOGURTE NATURAL', 'IOGURTE'), 'IOGURTE', 'IOGURTE NATURAL'), 'PEIXE CONSERVA', 'PEIXE EM CONSERVA')
			) a100 ON a100.ID_ORGAO = a15.ID_UNDD_ORGAO AND a100.EMPENHO = a20.CH_EMPN_EMPENHO
where	
	a12.ID_DT_ANO in (2020)
	and a20.CH_EMPN_EMPENHO in (130024000012020800471)
