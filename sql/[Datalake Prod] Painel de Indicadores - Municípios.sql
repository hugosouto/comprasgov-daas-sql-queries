WITH inativadas as (
	...
	FROM Stg_Siasg.siasg.siasg_unidade_gestora 	AS u
		JOIN Stg_Siasg.siasg.siasg_orgao		AS o ON u.IT_CO_ORGAO = o.IT_CO_ORGAO
	WHERE
	    u.IT_IN_STATUS <> ''
	    AND u.IT_DA_MOVIMENTO > '20210000'
	...
	WHERE
	    u.IT_IN_STATUS = ''
	    AND u.IT_DA_IMPLANTACAO_SIDEC > '20210000'
	    AND u.IT_IN_USO_SISG_UASG = 'S'
	    AND o.IT_NO_ORGAO <> ''
	 GROUP BY LEFT(u.IT_DA_IMPLANTACAO_SIDEC, 6)
)
SELECT DISTINCT
	COALESCE(c.anomes, i.anomes),
	...
	(SELECT count(*)
		...
		AND it_in_status = ''
		AND (it_da_implantacao_sidec < '20210000' or it_da_implantacao_sidec like '')) +
	(SELECT count(*)
		FROM Stg_Siasg.siasg.siasg_unidade_gestora s
		WHERE it_in_uso_sisg_uasg = 'S'
		AND it_in_status <> ''
		AND (it_da_implantacao_sidec < '20210000' or it_da_implantacao_sidec like '')
		AND it_da_movimento >= '20210000') as saldo_unidades,
	conta_unidades_cadastradas,
	conta_unidades_inativadas,
	getdate() AS datahoraregisto
