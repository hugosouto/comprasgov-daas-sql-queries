...
SELECT *
FROM analises.poc_item_deserto_fase_externa_v2 i
;

-- poc_item_deserto_original v2
SELECT 
	 i.id
	,i.numero_uasg
	,i.codigo_modalidade
...
--	,fase_compra
	...
	,nome_pdm_grupo
	,descricao
FROM analises.poc_item_deserto_base_v2 i
	LEFT JOIN catalogo.item_material m ON i.codigo_item_catalogo = m.codigo_item
	LEFT JOIN catalogo.item_servico s ON i.codigo_item_catalogo = s.codigo_servico
WHERE 1=1
--	AND situacao IN ('6')
...
-- poc_item_deserto_original v3 apenas material
SELECT 
	 i.id
	,i.numero_uasg
	,i.codigo_modalidade
...
FROM analises.poc_item_deserto_fase_externa_v3 i
	...
	LEFT JOIN catalogo.pdm_material_fornec pdm ON m.codigo_pdm = pdm.id_itcp_padrao_desc_mat
	LEFT JOIN catalogo.classe_material_fornec cls ON m.codigo_classe = cls.id_itcp_classe_mat_serv
WHERE i.tipo_item_catalogo = 'M'
;

-- Tabela Consolidada v3
SELECT
	 CAST(id AS VARCHAR) AS id_item
--	,numero_uasg -- (transformado)
	,codigo_modalidade -- (transformado)
...
--	,fase_compra
	...
	,md_fornec_por_pdm
FROM analises.poc_item_deserto_base_v3 i
	LEFT JOIN catalogo.item_material m ON i.codigo_item_catalogo = m.codigo_item
	LEFT JOIN catalogo.item_servico s ON i.codigo_item_catalogo = s.codigo_servico
WHERE 1=1
--	AND situacao IN ('6')
