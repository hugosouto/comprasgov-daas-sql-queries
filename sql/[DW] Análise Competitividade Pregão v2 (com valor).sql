WITH temp1 AS (
    SELECT	
      ...
      SUM(a15.VL_ITCP_PRECO_GLOBAL_ESTIM) 	AS VL_PRECO_TOTAL_ESTIM
...
SELECT
	...
	COUNT(pa12.ID_FRND_FORNECEDOR) 		AS CONTA_FORNECEDOR,
...
SELECT ID_DT_DATA AS data, CAST(AVG(CONTA_FORNECEDOR) AS FLOAT) AS mediaparticipantespregao, CAST(SUM(CONTA_FORNECEDOR) AS FLOAT) AS totalparticipantespregao, COUNT(DISTINCT SUBSTRING(COD_ITEM_GRUPO, 1, 17)) AS contacompras, COUNT(DISTINCT COD_ITEM_GRUPO) AS contaitensgrupos, SUM(VL_PRECO_TOTAL_ESTIM) AS valortotalestim, SUM(VL_PRECO_TOTAL_HOMOLOG) AS valortotalhomolog, NOW() AS datahoraregisto
FROM temp2
GROUP BY ID_DT_DATA
ORDER BY ID_DT_DATA;