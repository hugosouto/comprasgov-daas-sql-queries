...
SELECT DISTINCT in_agrupado, in_deserto, COUNT(*) contagem, COUNT(CAST(in_agrupado AS FLOAT))/1177071.0 /*(SELECT CAST(COUNT(*) AS FLOAT) FROM temp)*/ percentual
FROM temp
--WHERE in_agrupado = FALSE OR in_deserto = FALSE
--WHERE in_agrupado = TRUE AND in_deserto = TRUE
GROUP BY in_agrupado, in_deserto, CAST(in_agrupado AS FLOAT)
;