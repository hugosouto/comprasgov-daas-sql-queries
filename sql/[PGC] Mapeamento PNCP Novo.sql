...
-- 	JOIN pgc.historico_tramitacoes ON i.id = h.id_item
	JOIN (SELECT id_item, situacao, MAX(updated_at)
		  FROM pgc.historico_tramitacoes
		  GROUP BY id_item) h ON i.id = h.id_item 
WHERE
-- 	h.updated_at = MAX(h.updated_at) AND
-- 	h.situacao IN ('EN','EL') AND -- h.situacao = MAX(h.updated_at)
-- 	i.id_uasg = 837
	i.id_uasg = 3797
